...
def split_train_val(split_percent):
    ...
    df_list_train = []
    df_list_val = []
    for class_id, total_samples_class in enumerate(samples_per_class_df['file_name']):
        ...
        val_subset_class = df.loc[df['class_id']==class_id].groupby('class_id').tail(val_samples_class)
